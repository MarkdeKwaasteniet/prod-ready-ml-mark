"""Feature engineering for the animal shelter project."""
...
def add_features(df):
    ...
    df = df.assign(has_name=check_has_name(df["name"]))

    df = df.assign(sex=get_sex(df["sex_upon_outcome"]))

    df = df.assign(neutered=get_neutered(df["sex_upon_outcome"]))

    df = df.assign(hair_type=get_hair_type(df["breed"]))

    df = df.assign(days_upon_outcome=compute_days_upon_outcome(df["age_upon_outcome"]))
